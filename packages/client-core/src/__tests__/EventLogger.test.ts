...
describe('EventLogger', () => {
  ...
  const initFlushCoordinator = (logger: EventLogger) =>
    (logger as any)._initFlushCoordinator();
  ...
  describe('enqueue', () => {
    it('should add event to flush coordinator', () => {
      ...
      const coordinator = initFlushCoordinator(loggerWithLoggingEnabled);
      const event: StatsigEventInternal = {
        eventName: 'test-event',
        user: null,
        time: Date.now(),
      ...
      const addEventSpy = jest.spyOn(coordinator, 'addEvent');
      const checkQuickFlushSpy = jest.spyOn(coordinator, 'checkQuickFlush');
    ...
    it('should save event to storage when logging is disabled', () => {
      ...
      expect((eventLogger as any)._flushCoordinator).toBeNull();
  ...
  describe('setLoggingEnabled', () => {
    it('should load and enqueue stored events when switching from disabled to enabled', () => {
      ...
      const coordinator = initFlushCoordinator(eventLogger);
      const addEventSpy = jest.spyOn(coordinator, 'addEvent');
      const flushSpy = jest
    ...
    it('should update flush coordinator logging state', () => {
      const coordinator = initFlushCoordinator(eventLogger);
      const setLoggingSpy = jest.spyOn(coordinator, 'setLoggingEnabled');
  ...
  describe('stop', () => {
    ...
    it('should call processShutdown on flush coordinator', async () => {
      ...
      const processShutdownSpy = jest
        .spyOn(coordinator, 'processShutdown')
        .mockResolvedValue(undefined);
  ...
  describe('flush', () => {
    it('should delegate to flush coordinator', async () => {
      ...
      const processManualFlushSpy = jest
        .spyOn(coordinator, 'processManualFlush')
        .mockResolvedValue(undefined);
