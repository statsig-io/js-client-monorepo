...
function ManyGates() {
  ...
  useEffect(() => {
    performance.mark('precomputed-many-gate-checks-start');
    for (let i = 0; i < iterations; i++) {
      client.checkGate(`gate_num_${i}`);
    }
    performance.mark('precomputed-many-gate-checks-end');
    setMeasurement(
      performance.measure(
        'precomputed-many-gate-checks-duration',
        'precomputed-many-gate-checks-start',
        'precomputed-many-gate-checks-end',
      ),
    );
  }, []);
  ...
  return (
    <>
      {measurement && (
        <MeasurementDetails
          title={`${iterations} Gate Checks Measurement`}
